class DadosFundosController < ApplicationController
	...
	def pegarCnpjEChamarScrap
		if(params.has_key?(:cnpj))
			cnpj = params[:cnpj]
			puts "O CNPJ É: " + cnpj
			if (cnpj != "")
				listaFundos = DadosFundo.gerarDadosFundo(cnpj)
				DadosFundo.salvarDados(listaFundos)
				return listaFundos
			else
	...
	def dados_fundo_params
		params.require(:dados_fundo).permit(:codigoAtivo, :rendimento, :dataBase, :diaPagamento, :cnpj)
	end
end